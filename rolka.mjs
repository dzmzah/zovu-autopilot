...
async function graph(sciezka, params, metoda = 'POST') {
  const url = `${API}/${sciezka}`;
  const body = new URLSearchParams(params);
  const r = await fetch(metoda === 'GET' ? `${url}?${body}` : url, {
    method: metoda,
    body: metoda === 'GET' ? undefined : body,
  });
...
async function czekajNaKontener(id, token, minut = 5) {
  ...
  while (Date.now() < koniec) {
    const j = await graph(id, { fields: 'status_code,status', access_token: token }, 'GET');
...
async function naInstagram(url, tekst) {
  ...
  const kontener = await graph('me/media', {
    media_type: 'REELS',
    video_url: url,
    caption: tekst,
    share_to_feed: 'true',
  ...
  const wynik = await graph('me/media_publish', {
...
async function naFacebook(url, tekst) {
  ...
  const wynik = await graph(`${strona}/videos`, {
    file_url: url,
    description: tekst,
    access_token: token,
  });
