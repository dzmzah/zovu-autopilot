...
window.setT = (t) => {
  frazy.forEach((el, i) => {
    ...
    // ── первая фраза висит с НУЛЕВОГО кадра ──────────────────────
    // До этого начало ролика было пустым: почти полсекунды сток без текста
    // и без звука, потому что речь стартует на 0.45. В ленте это полсекунды
    // из трёх, которые вообще есть, — зритель успевает смахнуть, не поняв,
    // о чём речь. Замер это подтверждал: досмотр 21-25%.
    //
    // Поэтому хук показываем целиком и сразу, ровным светом. Караоке
    // подхватывает его, когда доходит голос: слово загорается плашкой, но
    // прочитать фразу можно было уже на первом кадре.
    if (i === 0 && t < f.w[0].a - 0.09) {
      el.style.visibility = 'visible';
      spany.forEach((s) => {
        s.style.opacity = 1;
        s.classList.remove('gra');
...
export async function zbuduj(plan) {
  ...
  //
  // НО это лечение для голоса из Veo. Дорожка из ElevenLabs приходит уже
  // живой: там и динамика есть, и верх на месте. Тот же экспандер с подъёмом
  // 2600 Гц на ней слышен как «ИИ-слоп» — Захар поймал это на слух. Своему
  // голосу оставляем только срез самого низа, остальное не трогаем.
  const GLOS = plan.glos
    ? 'highpass=f=80'
  ...
  if (music) {
    ...
    const chain = [
      ...
      `[2:a]aformat=channel_layouts=stereo,highpass=f=60,lowpass=f=2400,` +
        ...
        // не было цифровой тишины, а с фейдом сам её и создавал: первые
        // 0.1 с всё уходило ниже −50 дБ, и проверка называла это провалом.
        // Слышно его на пороге различимости, так что включать его резко
        // нечем — щёлкать там попросту нечему.
        `afade=t=out:st=${fade}:d=1.4[air]`,
    ...
    // Ключ тянем до конца ролика. `sidechaincompress` выдаёт ровно столько,
    // сколько идёт КОРОЧЕ из двух входов: кончился голос — оборвалась и
    // подложка. Это и есть мёртвый хвост, который был в каждом нашем ролике:
  ...
  return {
    ...
    // Секунда, с которой начинается фирменное аутро. Там по построению нет
    // голоса — только подложка на затухании, то есть минус 10-15 дБ к телу
    // ролика. Проверке это нужно знать, иначе она считает штатный хвост
    // провалом звука: 12.09 из-за этого встала сборка, а вышедшие раньше
    // ролики с таким же хвостом прошли только потому, что спад оказался
    // 12.5 дБ при пороге 13.
    ogonOd: plan.ogon ? +totalHero.toFixed(2) : null,
  };
}
