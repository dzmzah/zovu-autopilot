...
//
// ⚠️ ЭТО ДЕМО ТОЖЕ НЕ ОТПРАВЛЯТЬ — и вывод из него важнее самого демо.
//
// Проба показала предел движка: он силён в роликах про ИДЕЮ и РАСЧЁТ, где
// предметы — это слагаемые, и слаб в рекламе КОНКРЕТНОГО ТОВАРА. Здесь товар
// один, показать его нечем кроме эмодзи телефона, и тот повторяется трижды:
// в хуке как «разряженный», в середине как «большие кнопки», в финале как
// призыв. Предмет обязан говорить о своей фразе, а он говорит одно и то же.
// Плюс счётчик доезжает до 250 и стоит неподвижно семь секунд, и длина по
// слогам вышла 30 с против нашей планки 20.
//
// Что из этого следует для продаж: под рекламу товара просим у клиента ФОТО
// И ВИДЕО ПРОДУКТА. У Maxcom они точно есть — это производитель. С ними тот
// же сценарий станет сильным: телефон в руке, крупные кнопки, кнопка SOS
// живьём. Без них рисованный движок честно не вытягивает.
//
