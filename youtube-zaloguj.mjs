...
//
// Ключи клиента берутся из окружения: YOUTUBE_CLIENT_ID и
// YOUTUBE_CLIENT_SECRET. Они же лежат в секретах репозитория.
import { createServer } from 'node:http';
import { execFile } from 'node:child_process';

const ID = process.env.YOUTUBE_CLIENT_ID;
const SEKRET = process.env.YOUTUBE_CLIENT_SECRET;
if (!ID || !SEKRET) {
  console.error('нет YOUTUBE_CLIENT_ID / YOUTUBE_CLIENT_SECRET в окружении');
  process.exit(1);
}
