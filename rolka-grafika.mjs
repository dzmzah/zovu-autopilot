...
const FRAZY = [
  // Замер первого дубля: хук шёл 6,1 слог/с при планке 5,0 — то самое
  // «тараторит». Растягивать нельзя: правило простое — торопливую фразу
  // ПЕРЕПИСЫВАЕМ, а не лечим постобработкой, иначе слышно ИИ.
  //
  // Лечится точкой внутри фразы. Модель на точке делает настоящую паузу, та
  // входит в длительность, и темп падает сам — без единого фильтра. Заодно
  // ролик открывается одним словом, а это сильнее вопроса с разбега.
...
// кажутся нужными.
  ...
  // Думающее лицо ушло из хука совсем. Оно стояло на 880/1120 — ровно там,
  // где теперь крутится счётчик, и налезало на цифру. Двигать его некуда:
  // выше телефон, ниже подпись. Да и нужды в нём больше нет — раньше оно
  // изображало вопрос, а теперь вопрос задаёт цифра, и делает это лучше.
