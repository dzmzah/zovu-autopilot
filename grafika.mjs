 ...
 */
// Доля кадра, которую занимает предмет. Ставилась на глаз 17.08 — «крупные,
// а не мелкие иконки» — и вышло 820 px при ширине кадра 1080, три четверти.
// 23.08 Захар выбрал 70 % из сетки: предмет остаётся крупным, но перестаёт
// упираться в края и оставляет воздух вокруг.
//
// Множитель живёт ЗДЕСЬ, а не в числах сценариев: сценариев уже семь, и
// правка в каждом означала бы полсотни чисел и новый разъезд между ними.
// Живого видео он не касается — карточки с людьми и так небольшие.
export const SKALA_OBIEKTU = 0.7;

export function grafikaHtml(plan) {
  const esc = (s) => String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');

  // Уменьшаем один раз и отдаём дальше уже уменьшенное: разметка и скрипт
  // страницы читают размер из разных мест, и множитель в одном из них
  // означал бы предмет одного размера с тенью другого.
  const scena = plan.scena.map((o) =>
    o.obiekt ? { ...o, skala: Math.round((o.skala || 380) * SKALA_OBIEKTU) } : o
  );
...
/* Слова НЕ приглушаются прозрачностью. Приглушение осталось от старой
   схемы с жёлтой плитой: там непрозрачным было только звучащее слово, и
   остальные гасились. Когда пометкой стал ЦВЕТ, гашение потеряло смысл, а
   вред остался: 62 % белого на светлом полотне это не белый, а грязно-серый.
   В сетке вариантов, которую утвердил Захар, все слова были белыми — в
   ролике они такими не были. */
.slowo .s { display:inline-block; position:relative; padding:2px 14px;
  border-radius:14px; color:#fff; opacity:1;
...
/* Шкала стала высокой: подпись живёт ВНУТРИ неё, а не поверх. Тонкая
   полоса с текстом сверху — это две вещи, которые спорят за одно место;
   высокая полоса с текстом внутри — один предмет. */
.pasek-tor { position:absolute; left:0; right:0; top:6px; height:62px; border-radius:31px;
  background:rgba(255,255,255,.42); box-shadow:inset 0 2px 6px rgba(0,0,0,.14); }
.pasek-wype { position:absolute; left:0; top:6px; height:62px; border-radius:31px;
  background:#fff; box-shadow:0 8px 22px rgba(0,0,0,.22); }
/* Подпись — ТЁМНЫЙ текст внутри белой шкалы. Это третий заход: сначала
   белый текст с обводкой (грязно), потом белый с тенью на белом (каша),
   потом тёмная плашка поверх полосы (две плашки друг на друге). Правильно
   оказалось проще всего: полоса и есть плашка, текст просто ложится в неё.

   Кегль скромный, разрядка положительная — подпись здесь служебная, она
   называет единицу измерения, а не кричит. Кричит цифра на бирке выше. */
