...
async function powtorkiWLencie() {
  try {
    ...
    const OKNO_DNI = 14;
    // Повторы по 02.09 включительно — разобранная история, а не задача.
    // Всё, что вышло второго сентября, СОБИРАЛОСЬ ещё старой ротацией: она
    // при равном возрасте выбирала по порядку строк в массиве, а «ни разу не
    // выходил» было сразу у пятнадцати сценариев. Теперь решают замеры формы
    // плюс запрет на две одинаковые формы подряд.
    //
    // Дату сдвигаем, а не выключаем проверку: строка нужна для СЛЕДУЮЩЕГО
    // раза, когда банк кончится по-настоящему. Красный ран за уже вышедший
    // ролик, которого не отменить, две недели подряд — самый быстрый способ
    // научить человека пролистывать эту строку.
    const OD_NAPRAWY = Date.parse('2026-09-03T00:00:00Z');
