...
async function powtorkiWLencie() {
  try {
    ...
    const OKNO_DNI = 14;
    // Повторы до 02.09 — разобранная история, а не задача. До этой даты
    // ротация при равном возрасте выбирала по порядку строк в массиве, и
    // «ни разу не выходил» было сразу у пятнадцати сценариев: выбирала
    // случайность. Теперь при равном возрасте решают замеры формы, плюс
    // запрет на две одинаковые формы подряд.
    //
    ...
    const OD_NAPRAWY = Date.parse('2026-09-02T12:00:00Z');
